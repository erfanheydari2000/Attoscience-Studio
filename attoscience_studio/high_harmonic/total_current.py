# hihg_harmonic/total_current.py
...
##----------------------------------------------------
def tot_curr(lambda0_nm, filtering, window_func, file_path):
...
def tot_curr_connector(lambda0_nm, filtering, curr_components, plot_settings, window_func, extract_data_option, ipy_console=None):
    ...
    if "total_current" not in file_path.lower():
        ...
        return    
    if file_path:
        jx, jy, hx, hy, t, T = tot_curr(lambda0_nm, filtering, window_func, file_path)
        plot_tot_curr(jx, jy, hx, hy, t, T, filtering, lambda0_nm, curr_components, plot_settings, extract_data_option)
        ...
        msg = (
            ...
            + "--                          Total current log!                          --\n"
            + "-" * 75 + "\n"
            ">>> Total current plotted successfully!\n"
            f">>> File loaded from: {file_path}\n"
            f">>> Length of t: {len(t)}\n"
            f">>> Length of jx: {len(jx)}\n"
            f">>> Length of jy: {len(jy)}\n"
            + "-" * 75
        )

        print_to_console(ipy_console, msg)
##----------------------------------------------------
def plot_tot_curr(Jx, Jy, hx, hy, t, T, filtering, lambda0_nm, curr_components, plot_settings, extract_data_option):
    TIME_OC = t/T
    if 'x' in curr_components:
        if extract_data_option:
            file_path = "totcurr_x.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# Filtering:                {filtering}\n"
                f"# If you used the window function, the filtered total current is stored here.\n"
                "# t [a.u.], Jx [a.u.]"
            )
            np.savetxt(file_path, np.column_stack((t, hx)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(TIME_OC, Jx, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"),label='Jx (Original)')
        if filtering != 0:
            plt.plot(TIME_OC, hx, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "red"),label='Filtered Jx')
            plt.legend()  
        plt.title(plot_settings.get("graph_title", "Total Current (X)"))
        plt.xlabel(plot_settings.get("x_label", "Time [o.c.]"))
        plt.ylabel(plot_settings.get("y_label", "$J_{x}$ [a.u.]"))
    ...
    if 'y' in curr_components:
        if extract_data_option:
            file_path = "totcurr_y.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# Filtering:                {filtering}\n"
            ...
            np.savetxt(file_path, np.column_stack((t, hy)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(TIME_OC, Jy, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"),label='Jy (Original)')
        if filtering != 0:
            plt.plot(TIME_OC, hy, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "red"),label='Filtered Jy')                  
        plt.title(plot_settings.get("graph_title", "Total Current (Y)"))
        plt.xlabel(plot_settings.get("x_label", "Time [o.c.]"))
        plt.ylabel(plot_settings.get("y_label", "$J_{y}$ [a.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(TIME_OC)), right=plot_settings.get("x_max", np.max(TIME_OC)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
        plt.box(True)
        plt.show() 

    if '3D' in curr_components:
        if extract_data_option:
            file_path = "totcurr_3D.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# Filtering:                {filtering}\n"
            ...
            np.savetxt(file_path, np.column_stack((t, hx, hy)), header=header, comments='', fmt='%.12e')
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')        
        ax.plot(TIME_OC, Jx, Jy, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        ax.set_xlabel(plot_settings.get("x_label", "Time [o.c.]"))
        ax.set_ylabel(plot_settings.get("y_label", "$J_{x}$ [a.u.]"))
        ax.set_zlabel(plot_settings.get("z_label", "$J_{y}$ [a.u.]"))
...
##----------------------------------------------------
previous_input_totcurr = {}
class ModernDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Total Current Parameters")
    ...
    def create_header(self, layout):
        ...
        title = QLabel("Total Current Visualization")
        title.setStyleSheet("font-size: 22px; font-weight: bold; color: #1976d2; margin: 0;")
        
        subtitle = QLabel("Configure parameters for total current plotting")
    ...
    def create_required_section(self, layout):
        ...
        self.lambda0_entry.setText(str(previous_input_totcurr.get("lambda0_nm", "")))
        ...
        self.totcurr_x_checkbox = QCheckBox("X Component")
        self.totcurr_y_checkbox = QCheckBox("Y Component")
        self.totcurr_3D_checkbox = QCheckBox("3-Dimension")
    
        components_layout.addWidget(self.totcurr_x_checkbox)
        components_layout.addWidget(self.totcurr_y_checkbox)
        components_layout.addWidget(self.totcurr_3D_checkbox)
        ...
        self.filter_entry.setText(str(previous_input_totcurr.get("filtering", "0.0")))
    ...
    def create_optional_section(self, layout):
        ...
        #self.plot_options_checkbox.setIcon(QIcon("icons/display_settings_27dp_0000F5_FILL0_wght400_GRAD0_opsz24.png"))
        #self.plot_options_checkbox.setIconSize(QSize(24, 24))
    ...
    def create_axes_tab(self):
        ...
        axes_layout.addRow("Z-axis Range:", z_range_widget)
    ...
    def create_labels_tab(self):
        ...
        self.y_label_entry.setPlaceholderText("e.g., J_x [a.u.]")
        labels_layout.addRow("Y-axis Label:", self.y_label_entry)

        self.z_label_entry = QLineEdit()
        self.z_label_entry.setPlaceholderText("e.g., J_y [a.u.]")
        labels_layout.addRow("Z-axis Label:", self.z_label_entry)
    ...
    def on_submit(self):
        try:
            ...
            previous_input_totcurr["lambda0_nm"] = float(self.lambda0_entry.text())
        
            filtering = float(self.filter_entry.text())
            if not (0 <= filtering <= 100):
                raise ValueError("Filtering must be between 0 [%] and 100 [%].")
            previous_input_totcurr["filtering"] = float(self.filter_entry.text())
        
            curr_components = []
            if self.totcurr_x_checkbox.isChecked():
                curr_components.append('x')
            if self.totcurr_y_checkbox.isChecked():
                curr_components.append('y')
            if self.totcurr_3D_checkbox.isChecked():
                curr_components.append('3D')

            if not curr_components:
                QMessageBox.warning(dialog, "Invalid input", "Please select at least one J component.")
                return
            ...
            self.accept()
            # Update
            previous_input_totcurr.update({"lambda0_nm": lambda0_nm, "filtering": filtering})
            # CALL
            tot_curr_connector(lambda0_nm, filtering, curr_components, plot_settings, window_func, extract_data_option, self.parent().ipy_console)
            
        except ValueError as e:
            QMessageBox.warning(self, "Invalid Input", f"Error: {e}")

def input_dialog_tot_curr(parent):
