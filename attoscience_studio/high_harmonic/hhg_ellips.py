...
##----------------------------------------------------
def calcu_ellips(w, w0, Dx, Dy):       
    ww = w/w0
    a_right = (1/np.sqrt(2)) * (Dx + 1j * Dy)
    a_left = (1/np.sqrt(2)) * (Dx - 1j * Dy)
    epsilon = (np.abs(a_right) - np.abs(a_left)) / (np.abs(a_right) + np.abs(a_left))
    
    return epsilon
##----------------------------------------------------
def plot_HO_ellips(w, w0, SS, epsilon, lambda0_nm, q_value, T, extract_data_option, plot_settings):
    ww = w/w0
    if 'extract_data' in extract_data_option:
        file_path = "ho_ellips.txt"
        header = (
            f"# Driving wavelength [nm]: {lambda0_nm}\n"
            f"# T [a.u.]:                {T}\n"
            f"# w0 [a.u.]:               {w0}\n"
            f"# Max HO:                  {q_value}\n"
            "# ww (Harmonic order), SS (Intensity [arb.u]), ellipticity\n"
            + '#' * 60
        )
        np.savetxt(file_path, np.column_stack((ww, SS, epsilon)), header=header, comments='', fmt='%.12e')
    
    fig, ax1 = plt.subplots()
    
    ax1.plot(ww, SS, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "blue")) 
    
    ax1.set_xlabel(plot_settings.get("x_label", "Harmonic order"))
    ax1.set_ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"), color='blue')    
    
    ax1.tick_params(axis='y', labelcolor='blue')
    ax1.set_title(plot_settings.get("graph_title", "Ellipticity"))
    ax1.set_xlim(left=plot_settings.get("x_min", np.min(ww)), right=plot_settings.get("x_max", np.max(ww)))
    ax1.set_ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))

    ax2 = ax1.twinx()
    ax2.set_ylabel('ellipticity', color='red')    
    ax2.plot(ww, epsilon, linewidth=plot_settings.get("line_thickness", 1.2),color="red")
...
def ellips_connector(lambda0_nm, q_value, filtering, time_derivative, window_func, extract_data_option, plot_settings, ipy_console=None):
    ...
    if file_path:
        try:
            ...
            epsilon = calcu_ellips(w, w0, Dx, Dy)
            plot_HO_ellips(w, w0, SS, epsilon, lambda0_nm, q_value, T, extract_data_option, plot_settings)
            ...
            msg = (
                ...
                + "--                          Ellipticity log!                          --\n"
                + "-" * 75 + "\n"
                ">>> Ellipticity calculation and HHG visualization successfully completed!\n"
...
##----------------------------------------------------
previous_input_ellips = {}
class ModernDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Ellipticity Parameters")
    ...
    def create_header(self, layout):
        ...
        title = QLabel("Ellipticity of Hihg Harmonic Generation")
        title.setStyleSheet("font-size: 22px; font-weight: bold; color: #1976d2; margin: 0;")
        
        subtitle = QLabel("Configure parameters for ellipticity calculation from total current")
    ...
    def create_required_section(self, layout):
        ...
        self.lambda0_entry.setText(str(previous_input_ellips.get("lambda0_nm", "")))
        ...
        self.q_entry.setText(str(previous_input_ellips.get("q", "")))
        ...
        self.total_only_label = QLabel("Only total spectrum is meaningful for this calculation.")
        self.total_only_label.setStyleSheet("font-style: italic; color: #555555;")
        components_layout.addWidget(self.total_only_label)
        ...
        self.filter_entry.setText(str(previous_input_ellips.get("filtering", "0.0")))
    ...
    def on_submit(self):
        try:
            ...
            # Update
            previous_input_ellips.update({"lambda0_nm": lambda0_nm, "filtering": filtering, "q_value": q_value})
            
            # CALL
            ellips_connector(lambda0_nm, q_value, filtering, time_derivative, window_func, extract_data_option, plot_settings, self.parent().ipy_console)


        except ValueError as e:
            QMessageBox.warning(self, "Invalid Input", f"Error: {e}")

def input_dialog_ellips(parent):
