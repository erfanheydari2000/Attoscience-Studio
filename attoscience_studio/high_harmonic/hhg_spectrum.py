...
##----------------------------------------------------
def calculate_spectrum(lambda0_nm, q_value, filtering, window_func, time_derivative, file_path):
    ...
    wmax = q * w0
    dw = 0.001
    w = np.arange(0, wmax + dw, dw)
    #----
    Nomeg = len(w)
    Dx = np.zeros(Nomeg, dtype=np.complex128)
    Dy = np.zeros(Nomeg, dtype=np.complex128)
    Ttot = 1
    for m in range(Nomeg):
        integrand_x = np.exp(1j * w[m] * t) * dhx
        Dx[m] = np.trapz(integrand_x, t) / Ttot
        integrand_y = np.exp(1j * w[m] * t) * dhy
        Dy[m] = np.trapz(integrand_y, t) / Ttot
    #----
    Sx = w**2 * np.abs(Dx)**2
    Sx[Sx <= 0] = 1e-16
    Sx = np.log10(Sx)
    ...
    Sy = np.log10(Sy)
    #----
    S = w**2 * np.abs(Dx + Dy)**2
    S[S <= 0] = 1e-16
    SS = np.log10(S)   
    return w, Sx, Sy, SS, w0, Dx, Dy, t, T, Time_OC
##----------------------------------------------------
def plot_spectrum_harmonic_order(w, Sx, Sy, SS, w0, q_value, lambda0_nm, T, selected_spectrums, extract_data_option, plot_settings):
    ww = w / w0
    if 'total' in selected_spectrums:
        if extract_data_option:
            file_path = "hhg_tot.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# ww (Harmonic order), SS (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((ww, SS)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(ww, SS, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "HHG (Total)"))
        plt.xlabel(plot_settings.get("x_label", "Harmonic order"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(ww)), right=plot_settings.get("x_max", np.max(ww)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
        plt.box(True)
        plt.show() 
            
    if 'x' in selected_spectrums:
        if extract_data_option:
            file_path = "hhg_x.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                ...
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# ww (Harmonic order), Sx (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((ww, Sx)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(ww, Sx, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "HHG (x)"))
        plt.xlabel(plot_settings.get("x_label", "Harmonic order"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(ww)), right=plot_settings.get("x_max", np.max(ww)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
    ...
    if 'y' in selected_spectrums:
        if extract_data_option:
            ...
            header = (
                ...
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# ww (Harmonic order), Sy (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((ww, Sy)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(ww, Sy, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "HHG (y)"))
        plt.xlabel(plot_settings.get("x_label", "Harmonic order"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(ww)), right=plot_settings.get("x_max", np.max(ww)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
        ...
        plt.show()
##----------------------------------------------------
def plot_spectrum_energy(w, Sx, Sy, SS, w0, q_value, lambda0_nm, T, selected_spectrums, Ip_HeV, extract_data_option, plot_settings):
    www = w * Ip_HeV
    if 'total' in selected_spectrums:
        if extract_data_option:
            file_path = "ehhg_tot.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# www (Harmonic order), SS (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((www, SS)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(www, SS, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "Energy representation of harmonics (Total)"))
        plt.xlabel(plot_settings.get("x_label", "Energy [eV]"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(www)), right=plot_settings.get("x_max", np.max(www)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
        plt.box(True)
        plt.show() 
    ...
    if 'x' in selected_spectrums:
        if extract_data_option:
            file_path = "ehhg_x.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# www (Harmonic order), Sx (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((www, Sx)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(www, Sx, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "Energy representation of harmonics (x)"))
        plt.xlabel(plot_settings.get("x_label", "Energy [eV]"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(www)), right=plot_settings.get("x_max", np.max(www)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
    ...
    if 'y' in selected_spectrums:
        if extract_data_option:
            file_path = "ehhg_y.txt"
            header = (
                f"# Driving wavelength [nm]: {lambda0_nm}\n"
                f"# T [a.u.]:                {T}\n"
                f"# w0 [a.u.]:               {w0}\n"
                f"# Max HO:                  {q_value}\n"
                "# www (Harmonic order), Sy (Intensity [arb.u])\n"
                + '#' * 60
            )
            np.savetxt(file_path, np.column_stack((www, Sy)), header=header, comments='', fmt='%.12e')
        plt.figure()
        plt.plot(www, Sy, linewidth=plot_settings.get("line_thickness", 1.2),color=plot_settings.get("line_color", "black"))
        plt.title(plot_settings.get("graph_title", "Energy representation of harmonics (y)"))
        plt.xlabel(plot_settings.get("x_label", "Energy [eV]"))
        plt.ylabel(plot_settings.get("y_label", "Intensity [arb.u.]"))
        plt.xlim(left=plot_settings.get("x_min", np.min(www)), right=plot_settings.get("x_max", np.max(www)))
        plt.ylim(bottom=plot_settings.get("y_min", None), top=plot_settings.get("y_max", None))
...
def HHG_connector(lambda0_nm, filtering, q_value, time_derivative, selected_spectrums, window_func, extract_data_option, plot_settings,ipy_console=None):
    ...
    if file_path:
        try:
            ...
            plot_spectrum_harmonic_order(w, Sx, Sy, SS, w0, q_value, lambda0_nm, T, selected_spectrums, extract_data_option, plot_settings)
...
def EHHG_connector(lambda0_nm, q_value, filtering, time_derivative, selected_spectrums, window_func, Ip_HeV, extract_data_option, plot_settings,ipy_console=None):
    file_path, _ = QFileDialog.getOpenFileName(None, "Select total_current file")
    if "total_current" not in file_path.lower():
        QMessageBox.warning(None, "File Error", "Please upload the 'total_current' file.")
        return
    if file_path:
        try:
            w, Sx, Sy, SS, w0, Dx, Dy, t, T, Time_OC = calculate_spectrum(lambda0_nm, q_value, filtering, window_func, time_derivative, file_path)
            plot_spectrum_energy(w, Sx, Sy, SS, w0, q_value, lambda0_nm, T, selected_spectrums, Ip_HeV, extract_data_option, plot_settings)
...
class ModernDialog(QDialog):
    ...
    def create_header(self, layout):
        ...
        title = QLabel("Hihg Harmonic Generation")
    ...
    def create_required_section(self, layout):
        ...
        self.lambda0_entry.setText(str(previous_input_spectrum.get("lambda0_nm", "")))
        ...
        self.q_entry.setText(str(previous_input_spectrum.get("q", "")))
        ...
        self.filter_entry.setText(str(previous_input_spectrum.get("filtering", "0.0")))
        ...
        self.x_axis_unit_entry.addItems(["Harmonic order", "Energy [eV]"])
        ...
        x_axis_layout.addStretch()  # Push combo box to the left
    ...
    def on_submit(self):
        try:
            ...
            if self.spectrum_x_checkbox.isChecked():
                selected_spectrums.append('x')
            if self.spectrum_y_checkbox.isChecked():
                selected_spectrums.append('y')
            if self.spectrum_total_checkbox.isChecked():
                selected_spectrums.append('total')
            ...
            self.accept()
            # Update
            previous_input_spectrum.update({"lambda0_nm": lambda0_nm, "filtering": filtering, "q_value": q_value})
            
            # CALL
            if x_axis_unit == "Harmonic order":
                HHG_connector(lambda0_nm, filtering, q_value, time_derivative, selected_spectrums, 
                              window_func, extract_data_option, plot_settings,
                              self.parent().ipy_console)
            else:
                EHHG_connector(lambda0_nm, q_value, filtering, time_derivative, selected_spectrums, 
                               window_func, Ip_HeV, extract_data_option, plot_settings,
                               self.parent().ipy_console)
                
        except ValueError as e:
            QMessageBox.warning(self, "Invalid Input", f"Error: {e}")

def input_dialog_spectrum(parent):
