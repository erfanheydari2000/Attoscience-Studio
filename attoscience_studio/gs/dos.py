...
##----------------------------------------------------
def read_DOS(file_path):
    try:
        data = np.loadtxt(file_path)
        if data.size == 0:
            raise ValueError("The file is empty.")
        energy = data[:, 0]# In Hartree
        ...
        raise ValueError(f"Failed to read DOS: {e}")
##----------------------------------------------------
...
def DOS(plot_settings, ipy_console=None):
    file_path, _ = QFileDialog.getOpenFileName(None, "Select 'total-dos.dat' file")
    if "total-dos.dat" not in file_path.lower():
        QMessageBox.warning(None, "File Error", "Please upload the 'total-dos.dat' file.")
        return
    
    if file_path:
        try:
            energy, dos = read_DOS(file_path)
            plot_DOS(energy, dos, plot_settings)
            ...
            msg = (
                f">>> Time                          {timestamp}\n"
                + "-" * 75 + "\n"
                + "--                          Density of state log!                          --\n"
                + "-" * 75 + "\n"
                ">>> Density of state plotted successfully!\n"
            ...
            print(f"Error parsing DOS file: {e}")
            QMessageBox.critical(None, "Error", f"Failed to parse DOS file:\n{e}")
##----------------------------------------------------
def plot_DOS(energy, dos, plot_settings):
    plt.figure()
    plt.plot(dos,  energy, linewidth=plot_settings.get("line_thickness", 1.2),
             color=plot_settings.get("line_color", "black"))
    plt.xlabel(plot_settings.get("x_label", "Total density of state"))
    plt.ylabel(plot_settings.get("y_label", "Energy [a.u.]"))
    plt.title(plot_settings.get("graph_title", "Density of state"))
    plt.grid(False)
    plt.xlim(left=plot_settings.get("x_min", np.min(dos)), right=plot_settings.get("x_max", np.max(dos)))
...
class ModernDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Density of state Parameters")
    ...
    def create_header(self, layout):
        ...
        title = QLabel("Density of state Visualization")
        title.setStyleSheet("font-size: 22px; font-weight: bold; color: #1976d2; margin: 0;")
        
        subtitle = QLabel("Configure parameters for density of state plotting")
    ...
    def on_submit(self):
        try:
            ...
            # CALL
            DOS(plot_settings, self.parent().ipy_console) 
        except ValueError as e:
            QMessageBox.warning(self, "Invalid Input", f"Error: {e}")

def input_dialog_DOS(parent):
