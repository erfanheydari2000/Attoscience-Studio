...
##----------------------------------------------------         
def polarization_gating(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2, eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step, extract_data_option):
    ...
    T01 = 2 * np.pi / w01;T02 = 2 * np.pi / w02   
    T0_SI1 = T01*TIMEau;T0_SI2 = T02*TIMEau 
   
    Iau1 = np.sqrt(intensity1*1.0e12)/np.sqrt(3.509470*1.0e16)
    Iau2 = np.sqrt(intensity2*1.0e12)/np.sqrt(3.509470*1.0e16)
   
    E0_laser1 = np.sqrt(intensity1 * 10**12) / np.sqrt(3.509470 * 10**16)
    E0_laser2 = np.sqrt(intensity2 * 10**12) / np.sqrt(3.509470 * 10**16)
   
    A0_laser1 = Iau1*c_au/w01;A0_laser2 = Iau2*c_au/w02
    
    Ttot1 = cycles1 * T01;Ttot2 = cycles2 * T02
    alpha_L1 = alpha1 * np.pi / 180
    alpha_L2 = alpha2 * np.pi / 180
    ...
    Tend1 = Tst1 + Ttot1;Tend2 = Tst2 + Ttot2
   
    tm1 = Tst1+Ttot1/2;tm2 = Tst2+Ttot2/2
    dt = time_step
    t = np.arange(0, Tend2 + dt, dt) #hbar/H
    ...
    if envelope_name == 'Sine_square':
        ...
        Alaser_x1 = lambda t: np.real(+(np.cos(alpha_L1) + 1j * eps1 * np.sin(alpha_L1)) *  np.exp(1j * w01 * (t - Ttot1 / 2) + cep1) * envelope_func1(t))
        Alaser_x2 = lambda t: np.real(+(np.cos(alpha_L2) + 1j * eps2 * np.sin(alpha_L2)) *  np.exp(1j * w02 * (t - Ttot2 / 2) + cep2) * envelope_func2(t))
        Alaser_y1 = lambda t: np.real(+(np.sin(alpha_L1) - 1j * eps1 * np.cos(alpha_L1)) *  np.exp(1j * w01 * (t - Ttot1 / 2) + cep1) * envelope_func1(t))
        Alaser_y2 = lambda t: np.real(+(np.sin(alpha_L2) - 1j * eps2 * np.cos(alpha_L2)) *  np.exp(1j * w02 * (t - Ttot2 / 2) + cep2) * envelope_func2(t))
    ...
    At_x1 = np.array([Alaser_x1(ti) for ti in t])
    At_x2 = np.array([Alaser_x2(ti) for ti in t])
    At_y1 = np.array([Alaser_y1(ti) for ti in t])
    At_y2 = np.array([Alaser_y2(ti) for ti in t])
   
    envelope1 = np.array([envelope_func1(ti) for ti in t])
    envelope2 = np.array([envelope_func2(ti) for ti in t])
   
    At_x = At_x1 + At_x2
    At_y = At_y1 + At_y2

    return w01, w02, T01, T02, t, At_x, At_y, envelope1, envelope2

##----------------------------------------------------
def pg_fwhm(t, T01, T02, envelope1, envelope2):
    max_envelope1 = max(envelope1)
    max_envelope2 = max(envelope2)
   
    idx_max1 = np.argmax(envelope1)
    half_max_envelope1 = max_envelope1 / 2
    idx_max2 = np.argmax(envelope2)
    half_max_envelope2 = max_envelope2 / 2
    
    left_idx1 = np.where(envelope1[:idx_max1] <= half_max_envelope1)[0][-1]
    right_idx1 = np.where(envelope1[idx_max1:] <= half_max_envelope1)[0][0] + idx_max1
    
    left_idx2 = np.where(envelope2[:idx_max2] <= half_max_envelope2)[0][-1]
    right_idx2 = np.where(envelope2[idx_max2:] <= half_max_envelope2)[0][0] + idx_max2
    
    co_t_At1 = t / T01
    co_t_At2 = t / T02
    
    t_left1 = co_t_At1[left_idx1]
    t_left2 = co_t_At2[left_idx2]
    t_right1 = co_t_At1[right_idx1]
    t_right2 = co_t_At2[right_idx2]
    
    FWHM1 = t_right1 - t_left1
    FWHM_SI1 = (FWHM1 * T01) * TIMEau
    FWHM_SI_fs1 = FWHM_SI1*1.0e15
    FWHM2 = t_right2 - t_left2
    FWHM_SI2 = (FWHM2 * T02) * TIMEau
    FWHM_SI_fs2 = FWHM_SI2*1.0e15
    FWHM_t = FWHM_SI1

    return max_envelope1, max_envelope2, t_left1, t_right1, half_max_envelope1, FWHM_SI_fs1, FWHM_SI_fs2
##----------------------------------------------------
...
def pg_connector(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2, 
                 eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step,
                 plot_settings, extract_data_option,
                 ipy_console=None):
    try:
        w01, w02, T01, T02, t, At_x, At_y, envelope1, envelope2 = polarization_gating(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2, eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step, extract_data_option)

        max_envelope1, max_envelope2, t_left1, t_right1, half_max_envelope1, FWHM_SI_fs1, FWHM_SI_fs2 = pg_fwhm(t, T01, T02, envelope1, envelope2)
        
        pg_plot(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2, eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step, 
                w01, w02, T01, T02, 
                t, At_x, At_y, envelope1, envelope2, t_left1, t_right1, half_max_envelope1, FWHM_SI_fs1, plot_settings, extract_data_option)


        T01_SI = T01*2.418884326509*1e-17
        T02_SI = T02*2.418884326509*1e-17
        time_interv = max_envelope2 - max_envelope1
        ...
        msg = (
            ...
            + "--                          PG log!                          --\n"
            + "-" * 75 + "\n"
            ">>> PG calculation and visualization successfully completed\n"
            f">>> T1 [a.u.]: {T01:.12e}\n"
            f">>> T2 [a.u.]: {T02:.12e}\n"
            ...
            f">>> w1 [a.u.]: {w01:.12e}\n"
            f">>> w2 [a.u.]: {w02:.12e}\n"
            f">>> Envelope FWHM 1 [femtosecond]: {FWHM_SI_fs1:.12e}\n"
            f">>> Envelope FWHM 2 [femtosecond]: {FWHM_SI_fs2:.12e}\n" 
            f">>> Delay factor between two pulses: {delay:.12e}\n"
            f">>> Time interval between two peaks: {time_interv:.12e}\n"
            + "-" * 75
        )
...
def pg_plot(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2, eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step, 
                w01, w02, T01, T02, 
                t, At_x, At_y, envelope1, envelope2, t_left1, t_right1, half_max_envelope1, FWHM_SI_fs1, plot_settings, extract_data_option):
    if 'extract_data' in extract_data_option:
        ...
        header = (
            ...
            f"# Alpha 1 [deg]:             {alpha1}\n"
            f"# Alpha 2 [deg]:             {alpha2}\n"
            f"# CEP 1 [deg]:               {cep1}\n"
            f"# CEP 2 [deg]:               {cep2}\n"
            f"# Delay:                     {delay}\n"
            f"# T01 [a.u.]:                {T01}\n"
            ...
            "# t [a.u.],   At_x [a.u.],   At_y [a.u.],   envelope1 [a.u.],   envelope2 [a.u.]\n"
            + '#' * 80
        )
        np.savetxt(file_path, np.column_stack((t, At_x, At_y, envelope1, envelope2)), header=header, comments='', fmt='%.12e')
    
    plt.figure(1)
    bg_color = plot_settings.get("background_color", "white")
    plt.gcf().patch.set_facecolor(bg_color)

    plt.plot(t/T01 , At_x, linewidth=plot_settings.get("line_thickness", 1.4),color=plot_settings.get("line_color", "blue"), label='$A_{x}$')
    plt.plot(t/T01 , At_y, linewidth=plot_settings.get("line_thickness", 1.4),color=plot_settings.get("line_color", "red"), label='$A_{y}$')
    plt.grid(False)
    plt.title('Driving Laser: A(t)')
    plt.xlabel('time [o.c.]')
    plt.ylabel('A [a.u.]')
    plt.legend()
    plt.grid(False)
    ...
    plt.plot(t / T01, envelope1, linewidth=plot_settings.get("line_thickness", 1.4),color=plot_settings.get("line_color", "blue"))
    plt.plot(t / T02, envelope2, linewidth=plot_settings.get("line_thickness", 1.4),color=plot_settings.get("line_color", "red"))
    plt.plot([t_left1, t_right1], [half_max_envelope1, half_max_envelope1], color=[0, 0.7, 0.7], linewidth=1)
    plt.text((t_left1 + t_right1) / 2, half_max_envelope1 * 1.1, f'FWHM = {FWHM_SI_fs1:.2f} fs', horizontalalignment='center', color='y')
    ...
    ax.plot(t / T01, At_x, At_y, linewidth=plot_settings.get("line_thickness", 1.4),color=plot_settings.get("line_color", "black"))
    ax.set_xlabel('t / T0')
    ax.set_ylabel('$A_{x}$ [a.u.]')
    ax.set_zlabel('$A_{y}$ [a.u.]')
    ax.grid(False)
    plt.show()
...
class ModernDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Polarization Gating Parameters")
    ...
    def create_header(self, layout):
        ...
        title = QLabel("Polarization Gating")
        title.setStyleSheet("font-size: 22px; font-weight: bold; color: #1976d2; margin: 0;")

        subtitle = QLabel("Enter the parameters to create a laser pulse within the polarization gating framework.")
    ...
    def _create_pulse_parameters_section(self, parent_layout):
        ...
        pulse1_params = [
            ...
            ("alpha1", "Alpha:", "Enter the alpha (e.g., 20)", f"[{Symbols.DEGREE}]"),
            ("cep1", "CEP:", "Enter the CEP (e.g., 0.5)", f"[{Symbols.DEGREE}]")
        ]
        ...
        pulse2_params = [
            ...
            ("alpha2", "Alpha:", "Enter the alpha (e.g., 20)", f"[{Symbols.DEGREE}]"),
            ("cep2", "CEP:", "Enter the CEP (e.g., 0.5)", f"[{Symbols.DEGREE}]")
        ]
    ...
    def _create_display_options_section(self, parent_layout):
        ...
        self.time_step_spinbox.setValue(previous_input_pg.get("time_step", 0.2))
    ...
    def _add_parameter_fields(self, layout, params, pulse_num):
        ...
        for param_key, label, placeholder, unit in params:
            ...
            elif param_key == "alpha1":
                entry_attr_name = "alpha1_entry"
            elif param_key == "alpha2":
                entry_attr_name = "alpha2_entry"
            elif param_key == "cep1":
                entry_attr_name = "cep1_entry"
            elif param_key == "cep2":
                entry_attr_name = "cep2_entry"
            else:
    ...
    def _create_parameter_container(self, entry_attr_name, param_key, placeholder, unit):
        ...
        entry.setText(str(previous_input_pg.get(param_key, "")))
    ...
    def on_submit(self):
        try:
            ...
            previous_input_pg["lambda1_nm"] = lambda1_nm
    
            lambda2_nm = float(self.lambda2_nm_entry.text())
            if not (0 <= lambda2_nm <= 10000):
                raise ValueError("Driving wavelength must be positive and must be less than 10,000 nm.")
            previous_input_pg["lambda2_nm"] = lambda2_nm
            ...
            previous_input_pg["I_TWcm2_1"] = intensity1
            previous_input_pg["I_TWcm2_2"] = intensity2
            ...
            previous_input_pg["Cycles1"] = cycles1
            previous_input_pg["Cycles2"] = cycles2
            ...
            previous_input_pg["eps1"] = eps1
            previous_input_pg["eps2"] = eps2

            alpha1 = float(self.alpha1_entry.text())
            alpha2 = float(self.alpha2_entry.text())
            previous_input_pg["alpha1"] = alpha1
            previous_input_pg["alpha2"] = alpha2

            cep1 = float(self.cep1_entry.text())
            cep2 = float(self.cep2_entry.text())
            previous_input_pg["cep1"] = cep1
            previous_input_pg["cep2"] = cep2

            delay = float(self.delay_entry.text())
            previous_input_pg["delay"] = delay

            time_step = float(self.time_step_spinbox.value())
            if time_step <= 0.00001:
                raise ValueError("time_step cannot be less than 0.00001.")
            previous_input_pg["time_step"] = time_step

            envelope_name = self.envelope_entry.currentText()
            previous_input_pg["Envelope"] = envelope_name
            ...
            # Call
            pg_connector(lambda1_nm, lambda2_nm, intensity1, intensity2, cycles1, cycles2,
                        eps1, eps2, alpha1, alpha2, cep1, cep2, delay, envelope_name, time_step, 
                        plot_settings, extract_data_option,
